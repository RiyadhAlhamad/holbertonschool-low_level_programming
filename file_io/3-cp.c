...
int main(int ac, char **av)
{
    ...
    if (fdToo == -1)
    {
        dprintf(STDERR_FILENO, "Error: Can't write to %s. Check if the file exists and you have the proper permissions.\n", av[2]);
        exit(99);
    }
    while ((readed = read(fdFrum, buff, 1024)) > 0)
    {
        ...
        if (wrote == -1)
        {
            dprintf(STDERR_FILENO, "Error: Can't write to %s. Please check file permissions.\n", av[2]);
            exit(99);
        }
    }
    if (readed == -1)
